...
export default function ProductScreen(props) {
  ...
  const addToCartHandler = async () => {
    ...
    if (product.countInStock < quantity) {
      enqueueSnackbar("Ne pare rău, acest produs este epuizat", { variant: 'error' });
      return;
    }
    ...
    enqueueSnackbar(`${product.name} adăugat la coș`, { variant: 'success' });
          ...
          <Grid container spacing={1}>
            ...
            <Grid item md={6} xs={12}>
              <List>
                ...
                <ListItem>
                  <Typography variant="body1">Despre acest produs: {product.description}</Typography>
                </ListItem>
                <ListItem>
                  <Typography sx={{ paddingRight: '7px' }}>Categorie:</Typography>
                  <Chip label={product.category} color="success" variant="outlined" />
                </ListItem>
                <ListItem>
                  <Grid container>
                    <Grid item xs={6}>
                      <Typography component={'span'}>Preţ: ${product.price}</Typography>
                    </Grid>
                  </Grid>
                  <Grid item xs={6}>
                    <Typography component={'span'}>
                      {product.countInStock > 0 ?
                        (<Typography sx={{ fontWeight: 'bold', color: 'green' }}>În stoc</Typography>)
                        : (<Typography sx={{ fontWeight: 'bold', color: 'red' }}>Indisponibil</Typography>)}
                    </Typography>
                  </Grid>
                </ListItem>
                <ListItem>
                  <Grid item xs={12} md={12}>
                    ...
                    Adaugă in coş <AddShoppingCartIcon sx={{ marginLeft: '4px' }} />
                    </Button>
                  </Grid>
                </ListItem>
              </List>
