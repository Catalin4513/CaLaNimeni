...
function OrderScreen({ params }) {
  ...
  function onApprove(data, actions) {
    return actions.order.capture().then(async function (details) {
      try {
        ...
        enqueueSnackbar('Comanda a fost plătită!', { variant: 'success' });
        router.push('/success');
      } catch (err) {
  ...
  return (
    <Layout title={`Comanda ${orderId} | CaLaNimeni`}>
        ...
        : (<Grid container spacing={2}>
            <Grid item md={9} xs={12}>
              <Card sx={classes.section}>
                <List>
                  ...
                  <ListItem>
                    <Typography variant="subtitle1">
                      Număr Cameră: {userInfo.roomNumber}
                    </Typography>
                  </ListItem>
                  <ListItem>
                    Status: {' '}
                    {isDelivered ? 
                      `Livrat la ${deliveredAt}` : 
                      'Livrare în curs'}
                  </ListItem>
                </List>
              </Card>
            ...
            <Grid item md={3} xs={12}>
              <Card sx={classes.section}>
                <List>
                  <ListItem>
                    <Typography variant="h5" fontWeight="bold">Sumar Comandă</Typography>
                  </ListItem>
                  <ListItem>
                    <Grid container>
                      <Grid item xs={8}>
                        <Typography variant="subtitle1">
                          Subtotal 
                          <Typography variant="caption">({orderItems.reduce((a, c) => a + c.quantity, 0)} {orderItems.reduce((a, c) => a + c.quantity, 0) > 1 ? 'produse' : 'produs'})</Typography>
                        </Typography>
                      </Grid>
                      <Grid item xs={4}>
